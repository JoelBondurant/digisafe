...
pub mod android {
    ...
    #[no_mangle]
    pub extern "C" fn Java_com_digisafe_app_DroidKt_sha3(
        env: JNIEnv,
        _: JClass,
        arg: jbyteArray,
    ) -> jbyteArray {
        let arg: Vec<u8> = env.convert_byte_array(arg).unwrap();
        let hash = Sha3_256::digest(arg.as_slice());
        env.byte_array_from_slice(hash.as_slice()).unwrap()
    }

    #[no_mangle]
    pub extern "C" fn Java_com_digisafe_app_DroidKt_encrypt(
        env: JNIEnv,
        _: JClass,
        key: jbyteArray,
        nonce: jbyteArray,
    ...
    #[no_mangle]
    pub extern "C" fn Java_com_digisafe_app_DroidKt_decrypt(
        env: JNIEnv,
        _: JClass,
        key: jbyteArray,
        nonce: jbyteArray,
        ...
        let arg: Vec<u8> = env.convert_byte_array(arg).unwrap();
        let cipher = ChaCha20Poly1305::new(&key.into());
        let clear_txt = cipher.decrypt(&nonce.into(), arg.as_slice()).unwrap();
        env.byte_array_from_slice(clear_txt.as_slice()).unwrap()
    }
