use crate::db::{database::Unlocked, State};
...
fn build_ui(app: &gtk::Application) {
	let db = Arc::new(RwLock::new(State::UnlockedWrapper(Unlocked::new())));
	let status_bar = Rc::new(RefCell::new(gtk::Statusbar::new()));
	let side_margin = 20;

	let main_box = Rc::new(RefCell::new(
		gtk::Box::builder()
			.orientation(gtk::Orientation::Vertical)
			.build(),
	));

	let key_entry = Rc::new(RefCell::new(
		gtk::Entry::builder()
			.margin_top(20)
			.margin_bottom(10)
	...
	let val_entry = Rc::new(RefCell::new(
	...
	get_button.connect_clicked(move |_| {
		...
		if let State::UnlockedWrapper(ref db) = *db_get.read().unwrap() {
	...
	set_button.connect_clicked(move |_| {
		...
		let val = val_set
			.borrow()
			.buffer()
			.text(&bounds.0, &bounds.1, false)
			.to_string();
		if let State::UnlockedWrapper(ref mut db) = *db_set.write().unwrap() {
			db.set(key, val);
		}
		main_box2.borrow().set_sensitive(true);
	});
	...
	let (save_sender, save_receiver) =
		gtk::glib::MainContext::channel::<String>(gtk::glib::PRIORITY_DEFAULT);
	...
	save_button.connect_clicked(move |_| {
		...
		std::thread::spawn(move || {
			let msg = match &*db_save.write().unwrap() {
				State::UnlockedWrapper(db) => {
					let db_locked = db.clone().lock();
					db_locked.save();
					"Database saved".to_string()
				}
				State::LockedWrapper(_) => "Cannot save: database is locked".to_string(),
			};
	...
	let (unlock_sender, unlock_receiver) =
		gtk::glib::MainContext::channel::<String>(gtk::glib::PRIORITY_DEFAULT);
...
async fn unlock_dialog<W: gtk::glib::IsA<gtk::Window>>(
	window: Rc<W>,
	db: Arc<RwLock<State>>,
	...
	unlock_button.connect_clicked(move |_| {
		...
		std::thread::spawn(move || {
			let msg = {
				let mut state = dbcc.write().unwrap();
				if let State::LockedWrapper(ref locked_db) = *state {
					if let Some(unlocked_db) = locked_db.clone().unlock(raw_password.clone()) {
						*state = State::UnlockedWrapper(unlocked_db);
						"unlocked".to_string()
					} else {
						"unlock failed: incorrect password".to_string()
					}
				} else {
