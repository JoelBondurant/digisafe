				...
				match message {
					...
					Message::PasswordEntryGet => {
						if let Some(found_password_entry) = db.get_password_entry(query) {
							...
							*note =
								text_editor::Content::with_text(found_password_entry.get_note());
						} else {
					...
					Message::Tick => {
						if let Some(lit) = last_interaction_time {
							...
							let idle_remaining =
								idle_timeout.saturating_sub(idle_elapsed).as_secs();
						...
						let copy_timeout =
							Duration::from_secs(if cfg!(debug_assertions) { 8 } else { 20 });
						if let Some(lct) = *last_copy_time {
							let copy_elapsed = lct.elapsed();
							if copy_elapsed >= Duration::from_secs(4) {
								let copy_remaining =
									copy_timeout.saturating_sub(copy_elapsed).as_secs();
								*status =
									format!("Password copy time remaining: {copy_remaining}s");
							}
